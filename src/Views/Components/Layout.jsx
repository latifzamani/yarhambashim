...
function Layout({ children, window }) {
    ...
    const navItems =useMemo(()=>[
        ...
        { url: '/media', name: `${t('media')}`, icon: <PhotoAlbumOutlined /> },
                    ...
                    <Toolbar sx={{ justifyContent: 'space-between', paddingX: '16px' }}>
                        ...
                        {token ? (
                            <Typography variant='h6' component="div" sx={{ display: { xs: 'none', sm: 'block', md: 'block' }, color: 'black' }}>
                                <img src={logo} style={{ width:'7vh',height:'' }}/>
                            </Typography>
                        ) : (
                            <Link to='/login'>
                                <Typography variant='h6' component="div" sx={{ display: { xs: 'none', sm: 'block', md: 'block' }, color: 'black' }}>
                                    ...
                                    <RouterLink to={item.url} key={index} style={{}}>
                                        <Tab key={index}
                                            sx={{
                                                ...
                                                textTransform: 'none',
                                                textDecoration: location.pathname === item.url ? 'none' : 'none'
                                            }}
                                            label={item.name} />
                                    </RouterLink>
                                ))}
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', paddingX: '5vh', backgroundColor: '' }}>
                                    ...
                                    {token && (
                                        <RouterLink to='/dashboard'>
                                            <Button variant='text' startIcon={<MenuOutlined />} sx={{ width: '100%', color: 'black' }}>
                                                <Typography sx={{ marginX: '1vh', color: 'black' }}>{t('dashboard')}</Typography>
                                            </Button>
                                        </RouterLink>
                                    )}
                                </Box>
