  ...
  return (
    <CacheProvider value={createEmotionCache(direction)}>
      <Box sx={{display:'flex',flexDirection:'column',minHeight:'100vh'}}>
        ...
        <AppBar component="nav" sx={{width:'100%',backgroundColor:'white',}}>
            <Toolbar sx={{justifyContent:'space-between',paddingX:'16px'}}>
                ...
                <Box sx={{display:'flex',alignItems:'center',gap:3}}>
                  <Button variant='contained' sx={{backgroundColor:'green'}}>Donat</Button>
                {/* Language */}
                <Select value={language} onChange={(e)=>{setLanguage(e.target.value); i18n.changeLanguage(e.target.value)}}>
                  ...
                  <List sx={{textDecorationLine:'none',color:'black'}}>
                    {navItems.map((item,index)=>(
                      <RouterLink key={index} to={item.url}>
                        <Button variant='text' sx={{width:'100%'}}>{item.name}</Button>
                      </RouterLink>
                    ))}
                  </List>
            </Box>
        ...
        {/* Footer */}
        <Box component="footer" sx={{backgroundColor:'black',color:'white',py:2,mt:'auto'}}>
